...
def get_loaders():
    train_tf = transforms.Compose([
        ...
        transforms.ColorJitter(0.2, 0.2, 0.2, 0.1),  # <-- MOVED DOWN (now operates safely on Tensors)
    ...
    # Using the specific directory variables instead of DATASET_ROOT
...
class SimpleProj(nn.Module):
    """1x1 projection for variants without SRM."""
...
class PrototypeLayer(nn.Module):
    ...
    def forward(self, f):
        B, D, H, W = f.shape
        patches = f.view(B, D, -1).permute(0, 2, 1)          # BxSxD
        if self.similarity == 'cosine':
            fn = F.normalize(patches, p=2, dim=2)             # BxSxD
            pn = F.normalize(self.vectors, p=2, dim=1)        # PxD
            sim = torch.einsum('bsd,pd->bsp', fn, pn)         # BxSxP
            out, _ = sim.max(dim=1)
        else:   # l2
            f_sq  = (patches**2).sum(2, keepdim=True)          # BxSx1
            p_sq  = (self.vectors**2).sum(1).unsqueeze(0)       # 1xP
            cross = torch.einsum('bsd,pd->bsp', patches, self.vectors)
            d2    = (f_sq + p_sq - 2*cross).clamp(min=0)       # BxSxP
            # FIX: ProtoPNet log similarity — higher is more similar
            sim_l2 = torch.log((d2 + 1.0) / (d2 + 1e-4))
            out, _ = sim_l2.max(dim=1)                          # BxP
        return out
...
def apply_mixup_cutmix(imgs, targets, num_classes, device,
    ...
    Returns mixed imgs and soft targets (BxC floats).
    """
...
def cluster_loss(sim, hard_targets, K):
    ...
    """
    B, P = sim.shape
    proto_offsets = torch.arange(K, device=sim.device).unsqueeze(0)   # 1xK
    idx = hard_targets.unsqueeze(1) * K + proto_offsets                # BxK
    own_sims = torch.gather(sim, 1, idx)                                # BxK
...
def push_prototypes(model, loader, device):
    ...
    for imgs, _ in loader:
        feats = model.push_forward(imgs.to(device))    # BxDxHxW
        B, _, H, W = feats.shape
        patches = feats.view(B, D, -1).permute(0,2,1)  # BxSxD
