...
# =============================================================================
# CONFIG - OPTIMIZED FOR 38-CLASS DATASET
# =============================================================================
CONFIG = {
    # ── Paths (Configured for Kaggle Entishar Dataset) ─────────────────────────
    "TRAIN_DIR"             : "/kaggle/input/disease-dataset/new_disease_dataset/train",
    "VAL_DIR"               : "/kaggle/input/disease-dataset/new_disease_dataset/val",
    "TEST_DIR"              : "/kaggle/input/disease-dataset/new_disease_dataset/test",
    "SAVE_DIR"              : "/kaggle/working/saves/plantwild_powernet_38",

    # ── Dataset ────────────────────────────────────────────────────────────────
    "NUM_CLASSES"           : 38,        # Adjusted for the Entishar dataset
    ...
    "PROTOTYPE_DIM"         : 256,       # Higher capacity for fine-grained features
    ...
    "CE_WEIGHT"             : 1.0,
    "CLUSTER_WEIGHT"        : 1.0,
    "SEPARATION_WEIGHT"     : 0.2,       # Stronger push to separate confused classes
    "L1_WEIGHT"             : 1e-5,

    # ── Training phases ────────────────────────────────────────────────────────
    "PHASE1_EPOCHS"         : 10,        # Warmup
    "PHASE1_LR"             : 1e-3,

    "PHASE2_EPOCHS"         : 60,        # Main training loop
    "PHASE2_LR_BACKBONE"    : 2e-5,      # Very low for ConvNeXt to preserve ImageNet weights
    "PHASE2_LR_PROTO"       : 1e-3,
    "PHASE2_LR_FC"          : 1e-3,
    "PROTO_PUSH_EVERY"      : 2,         # Push prototypes more often

    "PHASE4_EPOCHS"         : 10,        # FC Fine-tune
    "PHASE4_LR_FC"          : 1e-5,

    # ── Anti-Overfitting ───────────────────────────────────────────────────────
    "ONLINE_AUG"            : True,
    "MIXUP_ALPHA"           : 0.0,       # DISABLED: Blending ruins XAI visual prototypes
    "CUTMIX_ALPHA"          : 0.0,       # DISABLED: Patching ruins XAI visual prototypes
    "DROPOUT"               : 0.3,
    "LABEL_SMOOTHING"       : 0.1,
    ...
    # ── Data loading ───────────────────────────────────────────────────────────
    "BATCH_SIZE"            : 16,        # Reduced to 16 to fit IMG_SIZE=384 in GPU memory
    "NUM_WORKERS"           : 4,
    "SAVE_EVERY"            : 5,
    "RANDOM_SEED"           : 42,
    "USE_AMP"               : True,
...
class Trainer:
    ...
    def _epoch(self, opt, use_l1=False, ce_only=False, apply_mixup=False):
        ...
        for imgs, lbls in bar:
            ...
            c1 += (preds == lbls).sum().item() # Accuracy calculated on original labels
            n += imgs.size(0)
            bar.set_postfix(loss=f"{total_l.item():.3f}", acc=f"{c1/n:.3f}")

        return {"loss": loss_sum / n, "top1_acc": c1 / n}

    def run(self):
        # Phase 1: Warmup
        log.info("── Phase 1: Warmup ──")
        opt1 = optim.AdamW([{"params": self.model.refinement.parameters(), "lr": CONFIG["PHASE1_LR"]},
