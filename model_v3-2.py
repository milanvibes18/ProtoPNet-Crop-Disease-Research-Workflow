...
CONFIG = {
    # ── Paths ──────────────────────────────────────────────────────────────────
    # Set these to match preprocess.py CONFIG["dst"]  (must be the same root)
    "TRAIN_DIR"             : "./Dataset/plantwild/train",   # ← match preprocess dst/train
    "VAL_DIR"               : "./Dataset/plantwild/val",     # ← match preprocess dst/val
    "TEST_DIR"              : "./Dataset/plantwild/test",    # ← match preprocess dst/test
    "SAVE_DIR"              : "./saves/plantwild_protopnet",

    # ── Dataset ────────────────────────────────────────────────────────────────
    "NUM_CLASSES"           : 80,        # ← CHANGED 98→80: matches preprocess top_k=80
    "IMG_SIZE"              : 300,       # ← CHANGED 224→300: EfficientNet-B3 native res
                                         #   preprocess saves all images at 300px
    ...
    "PROTOTYPE_DIM"         : 128,
    "PROTOTYPES_PER_CLASS"  : 7,         # reduced 10→7: less prototype overfitting surface
    "ADDON_CHANNELS"        : 256,
    ...
    # ── Early stopping ─────────────────────────────────────────────────────────
    "EARLY_STOP_PATIENCE"   : 8,         # increased 6→8: online aug makes val noisier
    ...
    # ── Online augmentation ────────────────────────────────────────────────────
    # Preprocess already produced offline-augmented copies at 300px.
    # Online aug adds stochasticity each epoch (prevents memorising static copies).
    # Scale min raised 0.5→0.7 so online crops don't clash too aggressively with
    # the offline RandomResizedCrop(300, scale=(0.6,1.0)) already applied.
    "TRAIN_CROP_SCALE_MIN"  : 0.7,       # ← CHANGED 0.5→0.7: offline aug already covers aggressive crops

    # ── MixUp ──────────────────────────────────────────────────────────────────
    "MIXUP_ALPHA"           : 0.2,       # Beta distribution param; 0.0 = disabled
...
class Utility:
    ...
    def get_train_transform():
        """Online augmentation applied live every epoch — prevents memorisation of
        static offline-augmented images and acts as strong regularisation."""
    ...
    def get_val_transform():
        """Deterministic transform for val and test — no randomness.
        Resize shorter edge then centre-crop: standard EfficientNet-B3 eval
        protocol at 300px (preprocess already saved images at 300px)."""
        sz = CONFIG["IMG_SIZE"]   # 300
        return transforms.Compose([
            transforms.Resize(int(sz * 1.067),    # ← CHANGED: scale then crop (320→300)
                              interpolation=transforms.InterpolationMode.LANCZOS),
            transforms.CenterCrop(sz),             # ← CHANGED: was bare Resize to (224,224)
            transforms.ToTensor(),
            transforms.Normalize(CONFIG["IMAGENET_MEAN"], CONFIG["IMAGENET_STD"]),
        ])

    # Keep for backward-compat (used by Model.predict / push_prototypes)
    @staticmethod
    def get_transform():
...
# MixUp
# =============================================================================
def mixup_batch(imgs: torch.Tensor, lbls: torch.Tensor, alpha: float):
    """
    MixUp augmentation (Zhang et al. 2018).
    Returns mixed images + two label tensors + lambda for computing mixed loss:

        imgs, lbl_a, lbl_b, lam = mixup_batch(imgs, lbls, alpha)
        loss = lam * criterion(logits, lbl_a) + (1 - lam) * criterion(logits, lbl_b)

    When alpha=0 lam==1, so behaviour is identical to no MixUp.
    """
...
class Trainer:
    def __init__(self, device):
        ...
        self.scaler = torch.amp.GradScaler("cuda", enabled=CONFIG["USE_AMP"])  # ← CHANGED: cuda.amp deprecated
    ...
    def _epoch(self, opt, use_l1=False, ce_only=False) -> dict:
        ...
        mix_prob  = CONFIG["MIXUP_PROB"]
        bar = tqdm(self.train_loader, desc="train", leave=False)
        for imgs, lbls in bar:
            imgs, lbls = imgs.to(self.device), lbls.to(self.device)

            # ── MixUp (Phase 2 joint training only; skip ce_only warm-up) ──
            use_mix = (not ce_only) and (alpha > 0) and (random.random() < mix_prob)
            if use_mix:
                imgs, lbl_a, lbl_b, lam = mixup_batch(imgs, lbls, alpha)
            # ──────────────────────────────────────────────────────────────

            opt.zero_grad()
            with torch.amp.autocast("cuda", enabled=CONFIG["USE_AMP"]):  # ← CHANGED: cuda.amp deprecated
                ...
                if ce_only:
                    ...
                    # Mixed CE replaces the standard CE term; prototype losses use lbl_a
                    ce_mix  = lam * self.criterion(logits, lbl_a) + (1 - lam) * self.criterion(logits, lbl_b)
                    _, _, clust, sep, l1 = Utility.compute_loss(
                        logits, dists, lbl_a, self.model, self.criterion, use_l1)
                    total_l = (ce_mix
                               + CONFIG["CLUSTER_WEIGHT"]    * clust
                               + torch.clamp(CONFIG["SEPARATION_WEIGHT"] * sep, min=-0.5)
                               + CONFIG["L1_WEIGHT"]         * l1)
                    ce = ce_mix
                else:
            ...
            # Accuracy measured against primary label (lbl_a when mixing)
            ref = lbl_a if use_mix else lbls
            c1 += (preds == ref).sum().item()
            top5 = logits.topk(min(5, logits.size(1)), 1).indices
            c5 += sum(ref[i] in top5[i] for i in range(len(ref)))
            n += imgs.size(0)
    ...
    def phase1(self):
        ...
        opt = optim.AdamW([                                              # ← CHANGED: Adam → AdamW
            {"params": self.model.addon.parameters(), "lr": CONFIG["PHASE1_LR_ADDON"],
             "weight_decay": CONFIG["WEIGHT_DECAY_ADDON"]},             # decoupled decay on addon
            {"params": self.model.fc.parameters(),    "lr": CONFIG["PHASE1_LR_FC"],
             "weight_decay": 0.0},                                       # no decay on fc during warm-up
        ])
    ...
    def phase2(self):
        ...
        opt = optim.AdamW([                                              # ← CHANGED: Adam → AdamW
            ...
            {"params": self.model.fc.parameters(),          "lr": CONFIG["PHASE2_LR_FC"],
             "weight_decay": 0.0},                                       # no decay on fc
        ])
    ...
    def phase4(self):
        ...
        opt = optim.AdamW([                                              # ← CHANGED: Adam → AdamW
            {"params": self.model.fc.parameters(), "lr": CONFIG["PHASE4_LR_FC"],
             "weight_decay": 0.0},                                       # L1 already regularises fc in phase4
        ])
