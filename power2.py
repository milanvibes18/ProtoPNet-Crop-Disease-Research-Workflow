...
CONFIG = {
    # ── Paths ──────────────────────────────────────────────────────────────────
    "TRAIN_DIR"             : "/kaggle/input/datasets/demodemo89/plantwild-top50/Output top k50/train",
    "VAL_DIR"               : "/kaggle/input/datasets/demodemo89/plantwild-top50/Output top k50/val",
    "TEST_DIR"              : "/kaggle/input/datasets/demodemo89/plantwild-top50/Output top k50/test",
    "SAVE_DIR"              : "/kaggle/working/saves/plantwild_protopnet_v3_4",
...
class Utility:
    ...
    def enforce_weight_constraint(model):
        iden_t = model.proto_layer.identity.T.bool()   
        with torch.no_grad():
            model.fc.weight.data = torch.where(
                iden_t,
                model.fc.weight.data.clamp(min=1e-4),   
                model.fc.weight.data.clamp(max=-1e-4),   
            )
