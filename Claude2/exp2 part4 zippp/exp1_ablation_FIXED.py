...
"""
EXPERIMENT 1 — ABLATION STUDY (FIXED)
=======================================
CHANGES FROM ORIGINAL:
  BUG FIX  : MixUp/CutMix was defined but never called in train_epoch().
              Now integrated correctly with soft-label cross-entropy.
  BUG FIX  : PrototypeLayer L2 branch used wrong formula sign for sim.
  ADDED    : Condition V7 — SE + L2 (isolates cosine vs L2 with SE present)
  ADDED    : Condition V8 — Full model, no MixUp/CutMix (augmentation ablation)
  ADDED    : Multi-seed support (N_ABLATION_SEEDS per condition)
  ADDED    : Parameter count + FLOPs reporting via torchinfo
  ADDED    : Generalisation gap column in results CSV
  ADDED    : ResNet-50 ProtoPNet baseline (V0) for bridge to literature

HOW TO USE:
  Set START_VARIANT / END_VARIANT per Kaggle session.
  Set N_ABLATION_SEEDS = 1 for quick run, 3 for paper-quality stats.
"""
...
# ─── CONFIG ───────────────────────────────────────────────────────────────────
DATASET_ROOT    = "/kaggle/input/datasets/entishar/disease-dataset/new_disease_dataset"
...
def get_loaders():
    train_tf = transforms.Compose([
        ...
        transforms.ToTensor(),                       # <-- MOVED UP
        transforms.ColorJitter(0.2, 0.2, 0.2, 0.1),  # <-- MOVED DOWN
    ...
    # Using the specific directory variables
