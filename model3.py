...
# =============================================================================
# CONFIG (KAGGLE VERSION - EFFICIENTNET-B3 NOVELTY)
# =============================================================================
CONFIG = {
...
class Utility:
    ...
    def compute_loss(logits, dists, labels, model, criterion, use_l1=False):
        ...
        cosine_mode = CONFIG["SIMILARITY_MODE"] == "cosine"
        for i in range(B):
            lbl = labels[i].item()
            d = dists[i]
            own = iden[:, lbl].bool()
            if cosine_mode:
                # cosine: higher = more similar; maximise own, minimise others
                if own.any():   clust -= d[own].max()
                if (~own).any(): sep  += d[~own].max()
            else:
                # l2 distance: lower = more similar; minimise own, maximise others
...
class AddOnLayers(nn.Module):
    def __init__(self, in_ch: int, proto_dim: int, mid_ch: int):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(mid_ch, proto_dim, 1, bias=False), nn.BatchNorm2d(proto_dim), nn.ReLU(),
            # ReLU (not Sigmoid): keeps non-negative activations without squashing
            # magnitude, which cosine normalisation handles anyway.
        )
...
if __name__ == "__main__":
    print("Initializing EfficientNet-B3 ProtoPNet on Kaggle...")
    M = Model()
    M.summary()
    M.train()
